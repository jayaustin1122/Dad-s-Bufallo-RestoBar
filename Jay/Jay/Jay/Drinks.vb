...
Public Class Drinks
    ...
    Sub Load_Foods()
        ...
        Try
            conn.Open()
            cmd = New MySqlCommand("SELECT `Dr_img`, `Dr_foodcode`, `Dr_name`, `Dr_price` FROM `drinks`", conn)
            dr = cmd.ExecuteReader
            While dr.Read
                Load_controls()
            End While
    ...
    Sub Load_controls()
        ...
        With pan
            ...
            .Tag = dr.Item("Dr_foodcode").ToString
        End With
        pan_top = New Panel
        With pan_top
            .Width = 170
        ...
        foodname.Text = " Drink Name : " & dr.Item("Dr_name").ToString
        price.Text = " Price              : ₱ " & dr.Item("Dr_price").ToString
    ...
    Public Sub Selectimg_Click(sender As Object, e As EventArgs)
        conn.Open()
        Try
            cmd = New MySqlCommand("SELECT `Dr_foodcode`, `Dr_name`, `Dr_price` FROM `drinks` WHERE `Dr_foodcode` like '" & sender.tag.ToString & "%' ", conn)
            dr = cmd.ExecuteReader
            While dr.Read
                Dim exist As Boolean = False, numrow As Integer = 0, numtext As Integer
                For Each itm As DataGridViewRow In DataGridView1.Rows
                    If itm.Cells(1).Value IsNot Nothing Then
                        If itm.Cells(1).Value.ToString = dr.Item("Dr_foodcode") Then
                            exist = True
                            numrow = itm.Index
                ...
                Next
                If exist = False Then
                    Dim price As Decimal = dr("Dr_price")
                    Dim subtotalprice As Double
                    subtotalprice = price * 1
                    DataGridView1.Rows.Add(DataGridView1.Rows.Count + 1, dr.Item("Dr_foodcode"), dr.Item("Dr_name"), dr.Item("Dr_price"), 1, subtotalprice)
    ...
    Private Sub textBoxRecieve_TextChanged(sender As Object, e As EventArgs)
        ...
        If MsgBox("Are You Sure?", vbQuestion + vbYesNo) = vbYes Then
            ...
            Else
                Try
                    ...
                    conn.Open()
                    cmd = New MySqlCommand("INSERT INTO `ordered`(`Co_id`, `Dr_id`, `Or_qty`, `Or_cost`, `Or_transNo`, `Or_date`) VALUES (@Coid, @Drid, @Orqty, @Orcost, @OrtransNo, @Ordate)", conn)
                    For j As Integer = 0 To DataGridView1.Rows.Count - 1 Step +1
                        cmd.Parameters.Clear()
                        cmd.Parameters.AddWithValue("@Coid", nextId)
                        cmd.Parameters.AddWithValue("@Drid", DataGridView1.Rows(j).Cells(1).Value)
    ...
    Private Function GetInventoryQuantity(productId As Integer) As Integer
        Dim connection As New MySqlConnection("server=localhost;user=root;password=;port=3306;database=dad's buffalo restobar")
        Dim commandText As String = "SELECT Dr_qty FROM drinks WHERE Dr_foodcode = @productId"
        Dim command As New MySqlCommand(commandText, connection)
    ...
    Private Sub UpdateInventoryQuantity(productId As Integer, newQuantity As Integer)
        Try
            Dim cmd As New MySqlCommand("UPDATE `drinks` SET `Dr_qty` = @quantity WHERE `Dr_foodcode`= @foodcode", conn)
            cmd.Parameters.Clear()
            cmd.Connection = conn
